...
// ============================================================
// cotizador_apu_bridge.js - integra APU de MAR DATA al cotizador
// ============================================================
...
(function () {
  ...
  function setResumen(item) {
    if (!item) {
      resumen.innerHTML = "Busca un APU y selecciónalo para cargarlo al cotizador.";
      return;
    }
    resumen.innerHTML = `
      <div><b>Concepto:</b> ${item.concepto || ""}</div>
      <div><b>Unidad:</b> ${item.unidad || ""}</div>
      <div><b>Precio unitario:</b> $${fmtMoney(item.precio_unitario)}</div>
      <div><b>Costo directo:</b> $${fmtMoney(item.costo_directo || 0)}</div>
      <div><b>Clave:</b> ${item.clave || ""}</div>
    `;
  }
  ...
  async function addAPUToQuote() {
    if (!selectedAPU) {
      Swal.fire("Selecciona un APU", "Primero busca y selecciona un APU.", "warning");
      return;
    }
    ...
    if (!row) {
      Swal.fire("Error", "No se pudo agregar una fila al cotizador.", "error");
      return;
    }
    ...
    if (!nombre || !unidad || !cantidadEl || !precio) {
      Swal.fire("Error", "La fila del cotizador no coincide con la estructura esperada.", "error");
      return;
    }
    ...
    // fuerza actualización total aunque el otro JS no haya escuchado aún
    document.getElementById("iva_porc")?.dispatchEvent(new Event("input", { bubbles: true }));

    Swal.fire({
      icon: "success",
      title: "APU agregado",
      text: "El concepto del APU se agregó al cotizador.",
      timer: 1200,
