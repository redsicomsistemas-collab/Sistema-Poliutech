...
sealed class LocalStore {
    ...
    public object Report(DateTimeOffset? from, DateTimeOffset? to, string? deviceId) { lock (gate) {
        ...
        var activity=selected.OrderByDescending(item=>item.StartedAt).Take(5000).Select(item=>{names.TryGetValue(item.DeviceId,out var device);return new{item.Id,item.DeviceId,employeeName=device?.EmployeeName??"Desconocido",computerName=device?.ComputerName??"",team=device?.Team??"",item.AppName,item.WindowTitle,item.Domain,item.StartedAt,item.EndedAt,activeSeconds=Math.Max(0,item.DurationSeconds-item.IdleSeconds),idleSeconds=Math.Min(item.DurationSeconds,item.IdleSeconds),item.Category};}).ToList();
        return new { from=start, to=end, summary=new { activeSeconds=active, idleSeconds=idle, productiveSeconds=productive, distractingSeconds=distracting, productivity=active==0?0:productive*100.0/active }, byDay, byPerson, applications, activity, activityRecordCount=selected.Count, activityRecordsReturned=activity.Count };
    }}
    TimeZoneInfo GetReportTimeZone(){try{return TimeZoneInfo.FindSystemTimeZoneById(settings.TimeZoneId);}catch{try{return TimeZoneInfo.FindSystemTimeZoneById("America/Mexico_City");}catch{return TimeZoneInfo.Local;}}}
